...
class Quest3:
    ...
    def get_wrist_pose(self, hand_side: str = "right") -> Optional[dict]:
        """Return the latest wrist pose in the converted right-handed frame.

        The returned pose is an absolute pose in the Quest tracking world.  The
        coordinate convention is x-forward, y-left, z-up, and the quaternion is
        ordered as ``[x, y, z, w]``.  ``timestamp`` uses ``time.monotonic()`` so
        callers can reject stale tracking data.
        """
        ...
        with self._lock:
            ...
            if (
                ...
                or hand.wrist_last_update is None
            ):
                return None
